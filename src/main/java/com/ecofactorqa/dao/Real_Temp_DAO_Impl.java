...
public class Real_Temp_DAO_Impl {
    ...
    public static int start_realtemp_thermostat_id_thermostat_event_heat = 0;
    public static int reschedule_real_temp_thermostat_id_thermostat_algoritm_heat = 0;  
	...
	public static void insertStartRealTempEntryHeat(int t_id, String next_phase_time_start, String date_setup_start, String execution_start_time_utc_start) {
		try {
			Class.forName("com.mysql.jdbc.Driver");
			Connection connection = DriverManager.getConnection(ef_11_db_url,ef_11_db_user,ef_11_db_pass);
		    Statement statment = connection.createStatement();
	...
	public static void start_real_temp_efts_heat(int t_id) {
		try {
			Class.forName("com.mysql.jdbc.Driver");
			Connection connection = DriverManager.getConnection(efts_db_url,efts_db_user,efts_db_pass);
		    Statement statment = connection.createStatement();

	        String sql = "SELECT * FROM ef_thermostat_event where thermostat_id= '" + t_id + "' and algorithm_id=131 and event_status='PROCESSED' and event_sys_time between timestamp (DATE_sub(now(), interval 300 SECOND)) and timestamp(now()) order by event_sys_time DESC limit 1;";	        
	        ResultSet result = statment.executeQuery(sql);
	        while (result.next()) {
	        	start_realtemp_thermostat_id_thermostat_event_heat = result.getInt("thermostat_id");
	        }      	        
		} 
		catch (SQLException se) {
			se.printStackTrace();
	...
	public static void real_temp_rescheduled_ef11_heat(int t_id) {
		try {
			Class.forName("com.mysql.jdbc.Driver");
			Connection connection = DriverManager.getConnection(ef_11_db_url,ef_11_db_user,ef_11_db_pass);
		    Statement statment = connection.createStatement();
